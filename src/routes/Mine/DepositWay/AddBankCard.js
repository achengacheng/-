...
import $ from 'jquery'
const CheckProgress = ({dispatch,getUserData}) => {
	...
	class CheckProgress extends React.Component{
		...
		render () {
			...
			return (
				<div>
                    <Header head_title='绑定银行卡' left_show='1' />
                    ...
                    <div className={styles.EditBox}>
						<ul className={styles.EditUl}>
							<li>
								<span>银行卡账户姓名</span>
								<input type="text" placeholder="请输入您的银行卡账户姓名" ref='cartName'/>
							</li>
							<li>
								<span>银行卡账号</span>
								<input type="number"  placeholder="请输入您的银行卡账户" ref='cardNO'/>
							</li> 
							<li>
								<span>请选择银行</span>
								<div className={styles.choiceBank} id='xuanBank'>
									<p>{!!this.state.reasons==false?'请选择银行':this.state.reasons}</p>
									&nbsp;&nbsp;
							...
							<li>
								<span>开户行名称</span>
								<textarea name="" id="textare2" onChange={this.textareac} placeholder="请输入您的银行卡开户行名称" ref='bankName'></textarea>
							</li>
						</ul>  
					</div>
